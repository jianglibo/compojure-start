(ns compojure-start.dutil.t-pressure-util
  (:require [clojure.test :refer :all]
            [midje.sweet :refer :all]
            [clojure.string :as cstr]
            [compojure-start.dutil.pressure-util :as pressure-util]
            [clojure.tools.logging :as log]
            [clj-http.client :as client]))


(against-background
 [(around :facts (let [prepared (pressure-util/prepare-users (atom {}) [[nil ["http://www.baidu.com"] 10 10]])] ?form))]
 (facts "userdefs test"
...
(fact "replace string"
      ...
      (count (pressure-util/random-str 8)) => 8
      (count (cstr/replace "abc{{rand}}cd{{rand}}" "{{rand}}" (pressure-util/random-str 8))) => 21)

(fact "test baidu home page"
      (let [urls ["http://www.baidu.com"]
            result-atom (pressure-util/benchmark [nil urls 10 10])]
        (count @result-atom) => 2
